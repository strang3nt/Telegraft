...
object StateMachine {
  ...
  final case class CreateChat(
      ...
      extends Command
  final case class GetUser(userId: Long, replyTo: ActorRef[Response])
      extends Command
  final case class CreateUser(userName: String, replyTo: ActorRef[Response])
      extends Command
  ...
  final case class JoinChat(
      ...
      replyTo: ActorRef[Response])
      extends Command
  final case class GetUserMessages(
      userId: Long,
  ...
  final case class Messages(userId: Long, messages: Seq[Message])
      extends Response
  final case class GetUserResponse(maybeUser: Option[User]) extends Response

  def apply(persistentRepository: ActorRef[PersistentUser.Command])
      : Behavior[Command] = Behaviors.receivePartial[Command] {
    case (ctx, command) => {
      ...
      command match {
        ...
        case CreateUser(username, replyTo) =>
          ctx.askWithStatus(
            ...
            case Success(_) =>
              WrappedResponse(
                ActionPerformed("User " + username + " created"),
                replyTo)
            case Failure(e) =>
              WrappedResponse(ActionFailed(e.getMessage), replyTo)
          }
          Behaviors.same
        case SendMessageTo(userId, receiverId, content, replyTo) =>
          Behaviors.same
